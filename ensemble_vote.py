...
class EnsembleVoteClassifier(BaseEstimator, ClassifierMixin, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        if self.weights.any() and len(self.weights) != len(self.clfs):
    ...
    def predict(self, X):
        ...
        if self.voting == 'soft':
            ...
            maj = np.apply_along_axis(lambda x:
                                      np.argmax(np.bincount(x.astype(int),
    ...
    def _predict(self, X):
        ...
        ##np.asarray([clf.predict(X) for clf in self.clfs_]).T
        _temp = np.asarray([clf.predict(X).flatten() for clf in self.clfs_]).T #problem i entered the flatten()
